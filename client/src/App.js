...
function App() {
  const RENDER_URL = "https://cyber-threat-detection-using-ai-final.onrender.com"; 

  // SMART API SELECTOR: Automatically switches between Localhost and Live Server
  const API_BASE = process.env.NODE_ENV === 'production' ? RENDER_URL : 'http://localhost:5000';
  ...
  const checkBreach = async (e) => {
    ...
    setTimeout(async () => {
        try {
            // USES THE SMART API_BASE URL
            const res = await axios.post(`${API_BASE}/api/search`, { email });
            ...
            alert(`Connection Failed!\n\nWebsite is trying to talk to: ${API_BASE}\n\n1. Check if Backend is running.\n2. If Live, check if Render URL is correct.`);
        }
        setLoading(false);
    }, 1500);
  };
    ...
    <div className="App">
      ...
      <nav className="navbar">
        ...
        <ul className="nav-menu">
            <li className={`nav-link ${activeTab === 'email' ? 'active' : ''}`} onClick={() => setActiveTab('email')}>Check Identity</li>
            <li className={`nav-link ${activeTab === 'password' ? 'active' : ''}`} onClick={() => setActiveTab('password')}>Passwords</li>
            <li className={`nav-link ${activeTab === 'notify' ? 'active' : ''}`} onClick={() => setActiveTab('notify')}>Notify Me</li>
            <li className={`nav-link ${activeTab === 'api' ? 'active' : ''}`} onClick={() => setActiveTab('api')}>API</li>
            <li className={`nav-link ${activeTab === 'demos' ? 'active' : ''}`} onClick={() => setActiveTab('demos')}>Demos</li>
            <li className={`nav-link ${activeTab === 'pricing' ? 'active' : ''}`} onClick={() => setActiveTab('pricing')}>Pricing</li>
      ...
      <div className="container">
        ...
        {activeTab === 'email' && (
            <>
                <form onSubmit={checkBreach} className="search-box">
                    <input type="email" placeholder="ENTER TARGET EMAIL ADDRESS..." value={email} onChange={(e) => setEmail(e.target.value)} required />
                    <button type="submit" className="scan-btn" disabled={loading}>{loading ? 'SCANNING...' : <><Search size={18} /> ANALYZE</>}</button>
                </form>
                {status && (
                <div className="dashboard-grid">
                    <div className="glass-panel status-section">
                        ...
                        {status === 'safe' ? (
                            <div className="status-badge badge-safe"><CheckCircle size={16} style={{marginRight:5}}/> SYSTEM SECURE</div>
                        ) : (
                            <div className="status-badge badge-danger"><AlertTriangle size={16} style={{marginRight:5}}/> BREACH DETECTED</div>
                        )}
                    </div>
                            ...
                            <div style={{marginBottom: '30px'}}>
                                ...
                                {breaches.map((b, i) => (
                                    <div key={i} className="breach-item">
                                        <div style={{display:'flex', justifyContent:'space-between'}}><h3>{b.Name}</h3><small>{b.BreachDate}</small></div>
                        ...
                        {(aiLoading || aiReport) && (
                            <div className="ai-container">
                                <div className="ai-header">
                                    ...
                                    {aiReport && <button className="download-btn" onClick={downloadPDF}><Download size={14}/> REPORT</button>}
                                </div>
                                {aiLoading ? (
                                    <div className="terminal-loader"><div className="terminal-line"> ESTABLISHING UPLINK TO GEMINI...</div></div>
                                ) : (
                                    <div>
                                        ...
                                        <ul className="action-list">{aiReport.actions.map((act, i) => <li key={i}>{act}</li>)}</ul>
                                    </div>
                                )}
                            </div>
                        )}
        ...
        {/* === TAB 5: DEMOS === */}
        {activeTab === 'demos' && ( <div className="glass-panel" style={{maxWidth: '1000px', margin: '0 auto', textAlign: 'left', padding: '40px'}}> <div style={{textAlign: 'center', marginBottom: '50px'}}> <h2 style={{fontSize: '2.5rem', fontFamily: 'Orbitron, sans-serif', marginBottom: '15px'}}>Demos</h2> </div> <div style={{display: 'flex', flexDirection: 'column', gap: '40px'}}> <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px', background: 'rgba(0,0,0,0.4)', padding: '30px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)'}}> <div> <h3 style={{color: 'white', marginTop: '0', fontSize: '1.8rem', fontFamily: 'Orbitron, sans-serif'}}>Domain Search</h3> <p style={{color: '#aaa', lineHeight: '1.6', fontSize: '1rem', marginBottom: '25px'}}> Domains searches are one of CyberGuard's most popular features. This demo walks through adding a domain to your dashboard. </p> <a href="https://www.youtube.com/watch?v=oi_xcOEx5uI" target="_blank" rel="noreferrer" style={{textDecoration: 'none'}}> <button style={{background: '#ff0000', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '5px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer'}}> <PlayCircle size={18} /> Watch on YouTube </button> </a> </div> <div style={{borderRadius: '10px', overflow: 'hidden', border: '1px solid #333', boxShadow: '0 10px 30px rgba(0,0,0,0.5)'}}> <iframe width="100%" height="250" src="https://www.youtube.com/embed/oi_xcOEx5uI" title="Domain Search Demo" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen ></iframe> </div> </div> </div> </div> )}

        {/* === TAB 6: PRICING === */}
        {activeTab === 'pricing' && ( <div style={{maxWidth: '1200px', margin: '0 auto', textAlign: 'center', padding: '20px'}}> <div style={{marginBottom: '50px'}}> <h2 style={{fontSize: '3rem', fontFamily: 'Orbitron, sans-serif', marginBottom: '15px'}}>Subscriptions</h2> </div> <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginBottom: '40px'}}> <div className="glass-panel" style={{padding: '30px', textAlign: 'center', border: '1px solid rgba(255,255,255,0.1)'}}> <h3 style={{fontSize: '1.8rem', color: 'white', margin: '0', fontFamily: 'Orbitron, sans-serif'}}>Pwned 1</h3> <div style={{color: '#888', marginBottom: '20px'}}>10 RPM</div> <div style={{fontSize: '2.5rem', fontWeight: 'bold', color: 'white', marginBottom: '10px'}}>₹380<span style={{fontSize: '1rem', color: '#888', fontWeight: 'normal'}}>/mo</span></div> <button style={{width: '100%', padding: '12px', background: '#0066ff', color: 'white', border: 'none', borderRadius: '5px', marginTop: '20px', cursor: 'pointer', fontWeight: 'bold'}}>Subscribe</button> </div> <div className="glass-panel" style={{padding: '30px', textAlign: 'center', border: '1px solid #00f3ff', background: 'rgba(0, 243, 255, 0.05)', transform: 'scale(1.05)'}}> <div style={{color: '#00f3ff', fontSize: '0.8rem', fontWeight: 'bold', marginBottom: '10px'}}>POPULAR</div> <h3 style={{fontSize: '1.8rem', color: 'white', margin: '0', fontFamily: 'Orbitron, sans-serif'}}>Pwned 2</h3> <div style={{color: '#888', marginBottom: '20px'}}>50 RPM</div> <div style={{fontSize: '2.5rem', fontWeight: 'bold', color: '00f3ff', marginBottom: '10px'}}>₹1,850<span style={{fontSize: '1rem', color: '#888', fontWeight: 'normal'}}>/mo</span></div> <button style={{width: '100%', padding: '12px', background: '#00f3ff', color: 'black', border: 'none', borderRadius: '5px', marginTop: '20px', cursor: 'pointer', fontWeight: 'bold'}}>Subscribe</button> </div> <div className="glass-panel" style={{padding: '30px', textAlign: 'center', border: '1px solid rgba(255,255,255,0.1)'}}> <h3 style={{fontSize: '1.8rem', color: 'white', margin: '0', fontFamily: 'Orbitron, sans-serif'}}>Pwned 3</h3> <div style={{color: '#888', marginBottom: '20px'}}>100 RPM</div> <div style={{fontSize: '2.5rem', fontWeight: 'bold', color: 'white', marginBottom: '10px'}}>₹3,150<span style={{fontSize: '1rem', color: '#888', fontWeight: 'normal'}}>/mo</span></div> <button style={{width: '100%', padding: '12px', background: '#0066ff', color: 'white', border: 'none', borderRadius: '5px', marginTop: '20px', cursor: 'pointer', fontWeight: 'bold'}}>Subscribe</button> </div> </div> </div> )}

        {/* === TAB 7: ABOUT PAGES === */}
        {activeTab === 'about' && ( <div className="glass-panel" style={{maxWidth: '800px', margin: '0 auto', textAlign: 'left', padding: '40px'}}> <h2 style={{fontFamily: 'Orbitron, sans-serif', color: 'white', fontSize: '2.5rem', marginBottom: '20px'}}>Who, What & Why</h2> <div style={{color: '#ccc', lineHeight: '1.8', fontSize: '1rem'}}> <p>Thanks for your interest in the CyberGuard AI Services. Further down this page you'll find the terms of use, written up by Australian lawyers, I just wanted to give a brief "friendlier" introduction before the formal bits.</p> <p>The intention of the terms of use is to ensure that CyberGuard AI is used, as I've always said, "to do good things after bad things happen". Data breaches are an unfortunate reality of increasingly online lives, but post-breach we can use that data to help reduce the impact on breach victims. Using our Services to do that is awesome, whereas using our Services to disadvantage breach victims, is not (that includes using it to pitch them services or "ambulance chase"). You're welcome to go and build amazing things that use our Services, my ask is that if you do that and display information from CyberGuard AI, that you clearly indicate the source.</p> <p>Most of what you'll read in the terms is obvious and common sense: don't deliberately attempt to disrupt the Services, don't redistribute your API key to other parties (this is your secret), if you don't pay a recurring invoice for the Services, they'll be cancelled and so on and so forth. By necessity, some of it is a bit "legal speak" which is why I wanted to set the context for why upfront.</p> <p>I hope you use our Services to create something wonderful that helps breach victims, thank you for reading this and for your interest in CyberGuard AI.</p> <p style={{marginTop: '20px'}}><strong>CyberGuard Founder</strong><br/>Founder and CEO<br/>CyberGuard AI</p> </div> </div> )}
        {activeTab === 'faq' && ( <div className="glass-panel" style={{maxWidth: '800px', margin: '0 auto', textAlign: 'left', padding: '40px'}}> <h2 style={{fontFamily: 'Orbitron, sans-serif', color: 'white', fontSize: '2.5rem', marginBottom: '30px', display: 'flex', alignItems: 'center', gap: '10px'}}> <HelpCircle size={32} color="#00f3ff"/> Frequently Asked Questions </h2> <div style={{display: 'flex', flexDirection: 'column', gap: '20px'}}> <details style={{background: 'rgba(255,255,255,0.05)', borderRadius: '8px', padding: '15px'}}> <summary style={{fontWeight: 'bold', color: '#00f3ff', cursor: 'pointer', fontSize: '1.1rem'}}>What does "compromised" mean?</summary> <p style={{color: '#aaa', marginTop: '10px', lineHeight: '1.6'}}>It means your data appeared in a data breach. It doesn't necessarily mean your account was accessed, but your credentials were exposed.</p> </details> <details style={{background: 'rgba(255,255,255,0.05)', borderRadius: '8px', padding: '15px'}}> <summary style={{fontWeight: 'bold', color: '#00f3ff', cursor: 'pointer', fontSize: '1.1rem'}}>What is a "breach"?</summary> <p style={{color: '#aaa', marginTop: '10px', lineHeight: '1.6'}}>A breach is an incident where data is inadvertently exposed in a vulnerable system, usually due to insufficient access controls or security weaknesses in the software.</p> </details> <details style={{background: 'rgba(255,255,255,0.05)', borderRadius: '8px', padding: '15px'}}> <summary style={{fontWeight: 'bold', color: '#00f3ff', cursor: 'pointer', fontSize: '1.1rem'}}>Are user passwords stored in this site?</summary> <p style={{color: '#aaa', marginTop: '10px', lineHeight: '1.6'}}>No. We do not store full passwords. For the password checker, we use k-Anonymity models to verify exposure without ever knowing your actual password.</p> </details> </div> </div> )}
        {activeTab === 'terms' && ( <div className="glass-panel" style={{maxWidth: '900px', margin: '0 auto', textAlign: 'left', padding: '40px'}}> <h2 style={{fontFamily: 'Orbitron, sans-serif', color: 'white', fontSize: '2.5rem', marginBottom: '20px'}}>Terms of Use</h2> <div style={{color: '#bbb', lineHeight: '1.6', height: '600px', overflowY: 'scroll', paddingRight: '15px'}}> <h4 style={{color: 'white'}}>1. Introduction</h4> <p>Welcome to CyberGuard AI, operated by Superlative Enterprises Pty Ltd trading as "CyberGuard AI" (we or us). We provide an online resource which facilitates the searching of email addresses, passwords and domains, allowing users to quickly assess if they, or people using their domain, may have been put at risk due to their online accounts having been compromised or "breached" in a data breach or leak.</p> <h4 style={{color: 'white'}}>2. PUBLIC GENERAL TERMS OF USE</h4> <p>You may use the site for personal, non-commercial purposes to check your own data exposure. By accessing and/or purchasing any of our Services, you agree to be bound by these Terms.</p> <h4 style={{color: 'white'}}>3. PAID SERVICES</h4> <p>If you purchase any Paid Services from us, additional clauses apply. Once you have subscribed to the Paid Services, we will grant you a non-exclusive, non-transferable and revocable licence for the Subscription Period to access and use our API and Domain Name Search function.</p> <h4 style={{color: 'white'}}>4. SUBSCRIPTION TERM</h4> <p>Your Subscription will commence on the date you purchase a Paid Service from us and it will continue to be valid for the period set out on your Dashboard (Subscription Period). Your Subscription Period will automatically renew for an additional period of the same length unless you cancel your Subscription prior to the end of the then current Subscription Period.</p> <h4 style={{color: 'white'}}>5. SUBSCRIPTION FEE</h4> <p>You must pay the Subscription Fee associated with your Subscription, as outlined on our Website. Fees are non-refundable except as required by law.</p> <h4 style={{color: 'white'}}>6. TAXES</h4> <p>You are responsible for paying all taxes, any other governmental fees and charges, and any penalties, interest, and other additions that are imposed on you relating to the payments made in accordance with these Terms.</p> <h4 style={{color: 'white'}}>7. INTELLECTUAL PROPERTY RIGHTS</h4> <p>You acknowledge and agree that we and our licensors, as applicable, are the owners of, and will retain all rights, title and interest in all Intellectual Property Rights in the Services.</p> <h4 style={{color: 'white'}}>8. DATA PROTECTION AND SECURITY</h4> <p>You agree that we will process your Personal Information to provide our Services in accordance with our privacy policy. You are responsible for establishing, maintaining and enforcing information security controls against the unauthorised access, destruction, loss, alteration, disclosure or misuse of Subscriber Data.</p> <h4 style={{color: 'white'}}>9. WARRANTIES AND DISCLAIMER</h4> <p>Your access to our Services is provided on an "as is" basis. We disclaim all other warranties (whether express, implied or statutory) and conditions, including fitness for purpose, availability, ongoing functionality, quality, accuracy, merchantability or non-infringement of our Services.</p> <h4 style={{color: 'white'}}>10. LIMITATION OF LIABILITY</h4> <p>To the fullest extent permitted by applicable law, neither party will be liable to the other party for any special, indirect or consequential loss.</p> <h4 style={{color: 'white'}}>11. INDEMNITY</h4> <p>You will indemnify us against any loss, damage, liability, charge, expense, outgoing or cost of any nature or kind, howsoever arising, whether present, unascertained, immediate, future or contingent arising out of or in connection with any claim arising from your use of our Services.</p> <h4 style={{color: 'white'}}>12. TERMINATION AND SUSPENSION</h4> <p>We may, with or without notice to you and at our discretion, limit or suspend your right to access or use our Services if we reasonably believe you are not complying with the Terms.</p> <h4 style={{color: 'white'}}>13. GENERAL</h4> <p>These Terms are governed by and must be construed in accordance with the laws in force in Queensland. The Terms contains the entire agreement between the parties concerning the subject matter of the agreement.</p> <h4 style={{color: 'white'}}>14. CHANGES TO THESE TERMS</h4> <p>We may, from time to time amend these Terms. We will endeavour to provide you with prior written notice on our Website of any such amendments.</p> <h4 style={{color: 'white'}}>15. DEFINITIONS AND INTERPRETATION</h4> <p>"Breach" refers to a data exposure incident. "User" refers to the individual accessing the site. "API" means the application programming interface.</p> <h4 style={{color: 'white'}}>16. OUR CONTACT DETAILS</h4> <p>Contact us at support@cyberguard.example.com.</p> </div> </div> )}
        {activeTab === 'privacy' && ( <div className="glass-panel" style={{maxWidth: '900px', margin: '0 auto', textAlign: 'left', padding: '40px'}}> <h2 style={{fontFamily: 'Orbitron, sans-serif', color: 'white', fontSize: '2.5rem', marginBottom: '20px'}}>Privacy Policy</h2> <div style={{color: '#ccc', lineHeight: '1.8', height: '600px', overflowY: 'scroll', paddingRight: '15px'}}> <p><strong>About us and what we do</strong></p> <p>CyberGuard AI is owned and operated by Superlative Enterprises Pty Ltd. This policy explains what limited personal information we collect when you use the CyberGuard website, the personal information we collect to provide our services and how we handle and protect your personal information.</p> <p><strong>What kinds of personal information do we collect and hold and why?</strong></p> <p>To provide our services we receive and collect information sets online by various methods which may include personal information which has or may have been the subject of a current or historical data breach or leak. We process these data sets to verify the legitimacy of breach and to identify new breached passwords.</p> <p>We do not collect or store your personal information when you conduct a search in the CyberGuard database. Searching for an email address or phone number only ever retrieves the data from storage then returns it in the response. The data result the search is not explicitly stored anywhere.</p> <p><strong>How do we protect your data?</strong></p> <p>Security on CyberGuard is handled by a "defence in depth" approach, employing many different layers of security including HTTPS transmission, Cloudflare protection, rate limits, regular security scans, and firewalls.</p> <p><strong>Access to and correction of your personal information</strong></p> <p>You may access the limited information we hold about you in the CyberGuard platform in real time. We don’t have any reasonable or practicable way to search for or retrieve and give access to any other personal information involved in a verified data breach.</p> <p><strong>Unsubscribing and opt-out</strong></p> <p>Every breach notification email that we send contains an unsubscribe link in the footer. Superlative provides an opt-out feature for CyberGuard that, if used, removes an email address from public visibility.</p> </div> </div> )}
        {activeTab === 'optout' && ( <div className="glass-panel" style={{maxWidth: '900px', margin: '0 auto', padding: '0', overflow: 'hidden'}}> <div style={{padding: '40px', textAlign: 'center'}}> <h2 style={{fontSize: '2.5rem', fontFamily: 'Orbitron, sans-serif', margin: '0'}}>Opt-Out</h2> <p style={{color: '#aaa', marginTop: '10px'}}>You can remove your email address from public search by opting out</p> </div> <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', borderTop: '1px solid rgba(255,255,255,0.1)'}}> <div style={{padding: '40px', textAlign: 'left'}}> <h3 style={{color: 'white', fontSize: '1.5rem', marginBottom: '15px'}}>Opt Out of Public Search</h3> <p style={{color: '#aaa', lineHeight: '1.6', fontSize: '0.95rem'}}> Opting out provides several options to ensure your email address is no longer publicly searchable. Once you verify control of the address, you'll be presented with three different models that let you decide how your visibility is removed — both from existing data breaches and any that may affect you in the future. </p> <p style={{color: '#aaa', marginTop: '20px', fontSize: '0.8rem'}}>Using CyberGuard AI is subject to the <span style={{color: '#00f3ff', textDecoration: 'underline', cursor: 'pointer'}} onClick={() => setActiveTab('terms')}>terms of use</span></p> </div> <div style={{background: 'rgba(0, 20, 40, 0.6)', padding: '40px', textAlign: 'left', borderLeft: '1px solid rgba(255,255,255,0.1)'}}> <h3 style={{color: 'white', fontSize: '1.3rem', marginBottom: '20px'}}>Enter your email address</h3> <div style={{display: 'flex', gap: '10px'}}> <input type="email" placeholder="Enter your email address" style={{ flex: 1, padding: '12px', borderRadius: '5px', border: 'none', background: 'white', color: 'black', width: '100%' }} /> <button style={{ background: '#0066ff', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '5px', fontWeight: 'bold', cursor: 'pointer', textTransform: 'uppercase', fontSize: '0.9rem' }}>opt-out</button> </div> <p style={{color: '#888', fontSize: '0.8rem', marginTop: '15px'}}>You will need to verify ownership of this email address</p> <div style={{marginTop: '25px', background: '#333', padding: '15px', borderRadius: '5px', display: 'flex', alignItems: 'center', gap: '10px', width: 'fit-content'}}> <div style={{background: '#00c853', borderRadius: '50%', padding: '2px'}}><Check size={16} color="white" strokeWidth={4}/></div> <div> <div style={{color: 'white', fontWeight: 'bold', fontSize: '0.9rem'}}>Success!</div> <div style={{color: '#aaa', fontSize: '0.7rem'}}>CLOUDFLARE</div> </div> </div> </div> </div> </div> )}
      ...
      <footer className="glass-footer">
        <div className="footer-content">
          ...
          <div className="footer-section">
            ...
            <a href="#" onClick={(e) => {e.preventDefault(); setActiveTab('password');}}>Passwords</a>
          ...
          <div className="footer-section">
            ...
            <div className="social-icons">
              ...
              <a href="mailto:babajanchand777@gmail.com"><Mail className="social-icon" size={20} /></a>
            </div>
